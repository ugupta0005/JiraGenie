...
const resultTitle = document.getElementById('resultTitle') as HTMLSpanElement;
const resultBody = document.getElementById('resultBody') as HTMLDivElement;
const resultDetails = document.getElementById('resultDetails') as HTMLDetailsElement;
const bugReportPreview = document.getElementById('bugReportPreview') as HTMLDivElement;
...
// ===== Progress Steps =====
function setStep(stepNum: number, status: 'active' | 'done' | 'reset'): void {
  const step = document.getElementById(`step${stepNum}`) as HTMLDivElement;
  const icon = step.querySelector('.step-icon') as HTMLSpanElement;
  if (status === 'reset') { step.className = 'step'; icon.textContent = '⏳'; }
  else if (status === 'active') { step.className = 'step active'; icon.textContent = '⟳'; }
  else { step.className = 'step done'; icon.textContent = '✓'; }
}

// ===== Analyze & Submit =====
analyzeBtn.addEventListener('click', async () => {
  const hasImage = selectedFiles.some(f => ALLOWED_IMAGE.includes(f.type));
  if (!hasImage) { showToast('Please add at least one screenshot for AI analysis', 'error'); return; }
  ...
  try {
    ...
    const result = await response.json() as {
      ...
      data?: {
        ...
        issueUrl: string;
        summary: string;
        attachments: Array<{ filename: string; size: number; success: boolean }>;
        bugReport: {
          title: string;
          severity: string;
